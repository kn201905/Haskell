...
## 型構築子（型コンストラクタ）
Haskell では `data` というキーワードを用いて新しい「型」を作ることができる。<br>
```Haskell
data BookInfo = Book Int String [String]
```

`data` に続く `BookInfo` は「型」の名前を表し、「型構築子（型コンストラクタ）」と呼ばれる。<br>
「型コンストラクタ」と呼ばれることが多いが、型を識別する単なる識別子として捉えたほうが分かりやすいと思う。

その後の `Book` は「値構築子（値コンストラクタ）」と呼ばれ、`Book` という識別子で `BookInfo` 型として利用できるデータを作る、という意味になる。<br>
`Book` に続く `Int String [String]` は、その型が持つデータは `Int` と `String` と `[String]` の３つとなる、という意味になる。<br>

「値構築子」の使い方は、以下のようになる。
```Haskell
book1 = Book 1 "Kids Science" ["Mike", "Bob"]  -- イメージとしては、本の ID、書籍名、複数の著者。
```
これで、`book1` は `BookInfo` 型の値で、上記の３つのデータを持つものとして利用できるようになる。

## 値構築子（値コンストラクタ）
C 言語などのプログラム言語を知っている人には、構築子が２つあることに違和感を感じると思う。<br>
Haskell は、複数のデータ構造を持つものを、１つの型で表せるようにしている。Haskell の「型」は、他の言語では discriminated union と呼ばれるもののイメージと一致する。

先程の例の `Book` 構築子は、「ID番号」と「書籍名」と「著者名」の３つのデータで `BookInfo` 型の値を作成した。<br>
「書籍名」と「価格」だけを持つデータを持つ `BookInfo` 型の値も作ることができる。その場合の値構築子を `BookPrice` とすると、以下のようにできる。<br>
`|` は、一般的な言語と同様に「or」の意味を表す。

```Haskell
data BookInfo = Book Int String [String]
              | BookPrice String Int
            
book1 = Book 1 "Kids Science" ["Mike", "Bob"]
book2 = BookPrice 2 "Kids Math" 500  -- 500円という価格の情報だけを持つデータ
```
上記の `book1` も `boo2` も `BookInfo` 型のデータとなる。

従って、`BookInfo` の型のデータを受け取ったとき、そのデータが必ず「著者名」のデータを持っている、などと考えてはいけない、と理解しておくこと。
...
## レコード構文
以下のようなレコード構文を用いて値構築子を記述すると、値構築子に渡す値に名前を付けられるようになる。
```Haskell
data BookInfo = Book {
   bookID :: Int,
   bookTitle :: String,
...
レコード構文を用いると、次のように値を生成することができる。
```Haskell
-- レコード構文を用いずに書くこともできる。
myBook1 = Book 1 "Pure Code" ["Mike", "Bob"]

-- レコード構文を用いて書くと、それぞれの値の意味が分かりやすい。
myBook2 = Book { bookID = 2, bookTitle = "Science", bookAuthors = ["John"] }
```

レコード構文に付けた各フィールド名は、**値を取り出す関数名** としても利用できるようになる。
```Haskell
ghci> :type bookTitle
bookTitle :: BookInfo -> String  -- BookInfo を受け取り、文字列を返す関数として認識されている。

ghci> bookTitle myBook2
"Science"
